...
title Musicode - Starting...

for %%I in ("%~dp0.") do set "ROOT=%%~fI"

echo.
echo   MUSICODE - Starting local dev environment
echo.
...
set "TMPBAT=%TEMP%\musicode-start-server.bat"
> "!TMPBAT!" echo @cd /d "!ROOT!\musicode-server" ^&^& mvn spring-boot:run ^> "!ROOT!\musicode-server.log" 2^>^&1
...
:backend_timeout
echo.
echo   [X] Backend did not start in 120s. Check musicode-server.log
exit /b 1
...
set "TMPBAT=%TEMP%\musicode-start-ui.bat"
> "!TMPBAT!" echo @cd /d "!ROOT!\musicode-ui" ^&^& npm run dev ^> "!ROOT!\musicode-ui.log" 2^>^&1
start "" /B cmd /c "!TMPBAT!"
...
:frontend_timeout
echo.
echo   [X] Frontend did not start in 30s. Check musicode-ui.log
exit /b 1
...
echo.
echo   Musicode is running!
echo.
echo     Frontend:  http://localhost:5173
echo     Backend:   http://localhost:8080
echo     Swagger:   http://localhost:8080/swagger-ui.html
echo.
echo   Logs: musicode-server.log / musicode-ui.log
echo   To stop: stop-musicode.bat
echo.
