...
|---|---|---|
| `sonance.admin.default-password` | `changeme` | Initial admin password |
| `sonance.jwt.secret` | dev key | HS256 signing key (≥32 chars) |
| `sonance.jwt.access-token-expiration-ms` | `900000` | Access token TTL (15 min) |
| `sonance.jwt.refresh-token-expiration-ms` | `604800000` | Refresh token TTL (7 days) |
| `sonance.cookies.secure` | `false` | Cookie Secure flag (true in Docker) |
| `sonance.lastfm.api-key` | _(empty)_ | Last.fm API key |
| `sonance.lastfm.api-secret` | _(empty)_ | Last.fm API secret |
| `sonance.encryption.token-key` | _(env)_ | AES-256-GCM key for scrobble tokens |
| `sonance.security.login-rate-limit.max-attempts` | `50` | Login attempts per window |
