 ...
 *
 * Key comes from MUSICODE_TOKEN_ENCRYPTION_KEY. Missing key fails fast at startup; the
...
public class TokenEncryptionService {
    ...
    public TokenEncryptionService(@Value("${musicode.encryption.token-key:}") String key) {
        if (!StringUtils.hasText(key)) {
            throw new IllegalStateException(
                "MUSICODE_TOKEN_ENCRYPTION_KEY is required. Generate with 'openssl rand -hex 32' and set before starting.");
        }
